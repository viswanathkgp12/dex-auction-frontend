...
if (scr1) {
  ...
  .state().commentary("setTimeout callback is queued as a task.")
...
new EventLoopAnimation(scr2)
    ...
    .state().showCodeBar().moveToLine(9).pushStack('Promise resolve callback')
    .state().pushLog()
    .state().popStack()
...
new EventLoopAnimation(scr3)
    ...
    .state().moveToLine(4).pushStack('setTimeout').commentary("Promise executor is processed instantly.")
    ...
    .state().pushTask(true).commentary("Time's up! Push setTimeout callback to task queue.")
    .state().hideCommentary().pushStack("setTimeout callback")
    .state().moveToLine(5).showCodeBar().pushMicrotask().commentary("Fulfill the promise, resolve callback is queued as a microtask.")
    .state().hideCodeBar().hideCommentary().popStack()
    .state().shiftTask()
    .state().activateMicrotask()
    .state().pushStack("Promise resolve callback").moveToLine(8).showCodeBar()
    .state().pushStack('console.log')
    .state().pushLog()
    .state().hideCodeBar().popStack()
    .state().popStack()
...
if (scr4) {
  new EventLoopAnimation(scr4)
    ...
    .state().hideCommentary().pushTask()
    .state().moveToLine(7)
    .state().commentary("Promise callbacks is queued as a microtask.")
    .state().hideCommentary().pushMicrotask()
    .state().moveToLine(13)
    .state().pushLog()
    .state().hideCodeBar().popStack()
